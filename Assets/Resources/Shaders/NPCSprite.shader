	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                float diff = length(abs((normalize(c.xyz) - normalize(_OriginalColor.xyz)))) < length(threshold);

                float3 outColor = lerp(c.xyz, _FloorColor.xyz, diff);
